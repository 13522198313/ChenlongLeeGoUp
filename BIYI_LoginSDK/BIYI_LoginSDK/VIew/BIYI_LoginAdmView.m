...
#import "UIView+CT.h"
#define GAPH 20
@interface BIYI_LoginAdmView()<UITextFieldDelegate>
...
@end
@implementation BIYI_LoginAdmView
...
- (void)changePIN{
    [_delegate BIYI_LoginAdmViewDelegateChangePin];
...
- (void)textFieldDidEndEditing:(UITextField *)textField{
  ...
  if (textField == self.usernameTextField) {
         [_delegate BIYI_LoginAdmViewDelegateWithUser:toBeString withPW:self.psTextField.text withPin:self.pinTextField.text];
  }else if (textField == self.psTextField){
          [_delegate BIYI_LoginAdmViewDelegateWithUser:self.usernameTextField.text withPW:toBeString withPin:self.pinTextField.text];
  }else{
       [_delegate BIYI_LoginAdmViewDelegateWithUser:self.usernameTextField.text withPW:self.psTextField.text withPin:toBeString];
  }
...
- (UIImageView *)userImageView{
    if (!_userImageView) {
        ...
        _userImageView.image = [UIImage imageNamed:MYFRAMEWORK_BUNDLE_IMG(@"BIYI_LoginView_username")];
    }
...
- (UIImageView *)pwImageView{
    if (!_pwImageView) {
        ...
        _pwImageView.image = [UIImage imageNamed:MYFRAMEWORK_BUNDLE_IMG(@"BIYI_LoginView_password")];
    }
    return _pwImageView;
}
...
- (UITextField *)psTextField{
    if (!_psTextField) {
        ...
        if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"BIYI_Login_SelectSavePW"] isEqualToString:@"1"]) {
               ...
               _psTextField.text = [[NSUserDefaults standardUserDefaults] objectForKey:@"BIYI_Login_PW"];
...
- (UIImageView *)pinImageView{
    if (!_pinImageView) {
        ...
        _pinImageView.image = [UIImage imageNamed:MYFRAMEWORK_BUNDLE_IMG(@"BIYI_LoginView_AdmPin_placeholderImage")];
    }
