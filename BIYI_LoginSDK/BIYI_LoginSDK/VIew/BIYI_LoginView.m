...
#define MYFRAMEWORK_BUNDLE_IMG(imageName) [@"BIYI_LoginSDK.bundle" stringByAppendingPathComponent:imageName]
#define GAPH 20
...
@interface BIYI_LoginView()<BIYI_LoginTypeViewDelegate>
...
/**用户名密码和手机号验证码自定义view*/
@property (nonatomic, strong) BIYI_LoginTypeView *loginTypeView;
/**勾选按钮和协议按钮*/
...
- (instancetype)initWithFrame:(CGRect)frame LoginType:(LoginType)loginType IsShowLoginAdmPinView:(BOOL)isShowLoginAdmPinView{
    if (self = [super initWithFrame:frame]) {
        if ([[NSUserDefaults standardUserDefaults] objectForKey:@"BIYI_Login_SelectSavePW"] == nil) {
                    [[NSUserDefaults standardUserDefaults] setObject:@"0" forKey:@"BIYI_Login_SelectSavePW"];
           
                    [[NSUserDefaults standardUserDefaults] synchronize];
                }
        
        if ([[NSUserDefaults standardUserDefaults] objectForKey:@"BIYI_Login_Agreement"] == nil) {
                           [[NSUserDefaults standardUserDefaults] setObject:@"0" forKey:@"BIYI_Login_Agreement"];
        ...
        if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"BIYI_Login_SelectSavePW"] isEqualToString:@"0"]) {
          
        }else{
            self.checkTwoBtn.selected = YES;
        }
        if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"BIYI_Login_Agreement"] isEqualToString:@"0"]) {
...
- (void)clickcheckBtn{
    ...
    if (self.checkBtn.selected) {
           NSLog(@"选择了同意协议");
           [[NSUserDefaults standardUserDefaults] setObject:@"1" forKey:@"BIYI_Login_Agreement"];
       }else{
           [[NSUserDefaults standardUserDefaults] setObject:@"0" forKey:@"BIYI_Login_Agreement"];
           NSLog(@"没选择了同意协议");
       }
...
- (void)clickCheckTwoBtn{
    ...
    if (self.checkTwoBtn.selected) {
        NSLog(@"选择了记住密码");
        [[NSUserDefaults standardUserDefaults] setObject:@"1" forKey:@"BIYI_Login_SelectSavePW"];
    }else{
        [[NSUserDefaults standardUserDefaults] setObject:@"0" forKey:@"BIYI_Login_SelectSavePW"];
        NSLog(@"没选择了记住密码");
    }
...
- (void)clickAgreementBtn{
    [_delegate BIYI_LoginViewDelegateClickAgreementBtn];
}
...
- (void)clickForgetPWDBtn{
    [_delegate BIYI_LoginViewDelegateClickForgetPWDBtn];
}
- (void)clickRegistBtn{
    [_delegate BIYI_LoginViewDelegateClickRegistBtn];
}



- (void)clickLoginBtn{
    if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"BIYI_Login_SelectSavePW"] isEqualToString:@"1"]) {
       [_delegate BIYI_LoginViewDelegateClickLoginBtnWithUser:userName withPW:[[NSUserDefaults standardUserDefaults] objectForKey:@"BIYI_Login_PW"] withPin:pinName];
    }else{
       [_delegate BIYI_LoginViewDelegateClickLoginBtnWithUser:userName withPW:passWord withPin:pinName];
    }
}
#pragma mark - BIYI_LoginTypeViewDelegate
- (void)BIYI_LoginTypeViewDelegateWithUser:(NSString *)user withPW:(NSString *)pw withPin:(NSString *)pin{
    ...
    if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"BIYI_Login_SelectSavePW"] isEqualToString:@"1"]) {
        [[NSUserDefaults standardUserDefaults] setObject:passWord forKey:@"BIYI_Login_PW"];
         
    }else{
      [[NSUserDefaults standardUserDefaults] setObject:@"" forKey:@"BIYI_Login_PW"];
    }
}

- (void)BIYI_LoginTypeViewDelegateGetPinBtn{
    [_delegate BIYI_LoginViewDelegateGetPinBtn];
   
}

- (void)BIYI_LoginTypeViewDelegateChangePin{
    [_delegate BIYI_LoginViewDelegateChangePin];
}
#pragma mark - alloc init
- (UIImageView *)bgView{
    if (!_bgView) {
        _bgView = [[UIImageView alloc] initWithFrame:self.frame];
        _bgView.image = [UIImage imageNamed:MYFRAMEWORK_BUNDLE_IMG(@"BIYI_Login_bg")];
    }
    return _bgView;
}
...
- (UIImageView *)logoView{
    if (!_logoView) {
        ...
        _logoView.image = [UIImage imageNamed:MYFRAMEWORK_BUNDLE_IMG(@"BIYI_Login_logo")];
    }
    return _logoView;
}
...
- (UIButton *)telBtn{
    if (!_telBtn) {
        ...
        [_telBtn setBackgroundImage:[UIImage imageNamed:MYFRAMEWORK_BUNDLE_IMG(@"BIYI_LoginView_Adm_normal")] forState:UIControlStateNormal];
        [_telBtn setBackgroundImage:[UIImage imageNamed:MYFRAMEWORK_BUNDLE_IMG(@"BIYI_LoginView_Adm_select")] forState:UIControlStateSelected];
...
- (UIButton *)admBtn{
    if (!_admBtn) {
        ...
        [_admBtn setBackgroundImage:[UIImage imageNamed:MYFRAMEWORK_BUNDLE_IMG(@"BIYI_LoginView_Pin_normal")] forState:UIControlStateNormal];
        [_admBtn setBackgroundImage:[UIImage imageNamed:MYFRAMEWORK_BUNDLE_IMG(@"BIYI_LoginView_Pin_select")] forState:UIControlStateSelected];
...
- (BIYI_LoginTypeView *)loginTypeView{
    if (!_loginTypeView) {
        _loginTypeView = [[BIYI_LoginTypeView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.displayNameLab.frame) + GAPH + self.admBtn.frame.size.height + GAPH, self.frame.size.width, self.height/4)];
...
- (UIButton *)checkBtn{
    if (!_checkBtn) {
        ...
        [_checkBtn setBackgroundImage:[UIImage imageNamed:MYFRAMEWORK_BUNDLE_IMG(@"BIYI_LoginView_checked")] forState:UIControlStateSelected];
        [_checkBtn setBackgroundImage:[UIImage imageNamed:MYFRAMEWORK_BUNDLE_IMG(@"BIYI_LoginView_nochecked")] forState:UIControlStateNormal];
...
- (UIButton *)checkTwoBtn{
    if (!_checkTwoBtn) {
        _checkTwoBtn = [[UIButton alloc] initWithFrame:CGRectMake(CGRectGetMaxX(self.agreementBtn.frame), self.agreementBtn.frame.origin.y, 20, 20)];
            ...
            [_checkTwoBtn setBackgroundImage:[UIImage imageNamed:MYFRAMEWORK_BUNDLE_IMG(@"BIYI_LoginView_checked")] forState:UIControlStateSelected];
            [_checkTwoBtn setBackgroundImage:[UIImage imageNamed:MYFRAMEWORK_BUNDLE_IMG(@"BIYI_LoginView_nochecked")] forState:UIControlStateNormal];
